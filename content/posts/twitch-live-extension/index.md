...
summary = 'A browser extension that helps you track when your favorite Twitch streamers go live and sends notifications.'
...
It's available for every Chromium based browser (Chrome/Edge/Arch, etc) on the [Chrome Web Store](https://chromewebstore.google.com/detail/twitch-live-extension/nlnfdlcbnpafokhpjfffmoobbejpedgj)
...
## [Version 3 - Twitch API improvements](https://github.com/PedroS11/twitch-live-extension/pull/12)
...
## [Version 4.3 - Firefox addon on Manifest Version 3](https://github.com/PedroS11/twitch-live-extension/pull/80)
...
- If it's Chrome, leave the communication as it was before:
   - if the token wasn't already saved in local storage, 
fetch it from the background, return it to the popup via Chrome message API, and save it in local storage.
- If it's Firefox, save the token in browser storage when the background script runs. Then, every time the popup is opened, it will fetch the token from browser storage.
